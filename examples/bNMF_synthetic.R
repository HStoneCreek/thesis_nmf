...
df <- read.csv('synthetic_data_90_300_3_42.csv')


y = as.matrix(df)

run_nmf_model <- function(seed, K, dropout = 0.1) {
  ...
  nmf_data3 <- list(
    ...
    I = 300, # This might need to be adjusted if it also needs to reflect changes in the dataset size
    K = K,
    y = y,
    a_start = a,
    b_start = 20
  ...
  results_filename <- paste0("r_synthetic/synthetic_seed_", seed, "_K_", K, "_dropout_", dropout, "12345.csv")
  write.csv(theta_hat3, file = results_filename)
  if (dropout > 0 && dropout < 1) {
  # Save the permutation
      perm_filename <- paste0("~/PycharmProjects/nmf/genolytics/bnmf/r_synthetic/permutation_synthetic_seed_", seed, "_K_", K, "_dropout_", dropout, ".csv")
  ...
  ggsave(filename = "posterior_density_plot_improved.png", plot = p, width = 20, height = 12, dpi = 300, bg = "white")
...
# Example usage:
seeds <- c(12345)#, 23456, 34567, 45678, 56789)
for (seed in seeds) {
  df <- run_nmf_model(seed, K = 9, dropout=2)
}
